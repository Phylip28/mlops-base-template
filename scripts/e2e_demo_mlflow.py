...
def run():
    ...
    for uc in USE_CASES:
        ...
        for i in range(35):
            payload = {"features": {"a":random.random(),"b":random.random(),"c":random.random()}, "target": 1}
            try:
                r = requests.post(f"{API_URL}/ingest/{uc}", json=payload)
                if i % 10 == 0:
                    print(f"  Enviado req {i}, status: {r.status_code}, response: {r.text}")
    ...
    for uc in USE_CASES:
        try:
            ...
            if exp:
                ...
                if latest:
                    print(f"✅ {uc} entrenado. Accuracy = {latest.data.metrics.get('accuracy')}")
                else: 
