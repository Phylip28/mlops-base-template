...
class MultiTenantAutoMLOrchestrator:
    ...
    def _trigger_retraining(self, use_case: str, df: pd.DataFrame, file_path: str):
        ...
        with mlflow.start_run(run_name=f"auto_retrain_{use_case}") as run:
            print(f"\n[MLOps] ----------------------------------------------------")
            print(f"[MLOps] Entrenando modelo REAL para {use_case} con {len(df)} eventos...")
            ...
            mlflow.sklearn.log_model(model, "model_artifact", registered_model_name=model_name)
            
            # Obtener ultima version para promoverla
            latest_version = client.get_latest_versions(model_name, stages=["None"])[0].version
            client.set_registered_model_alias(model_name, "champion", latest_version)
            
            print(f"[MLOps] Modelo '{model_name}' (v{latest_version}) guardado en MinIO y MLflow!")
            print(f"[MLOps] Exactitud (Accuracy) obtenida: {acc:.2f}")
            print(f"[MLOps] ----------------------------------------------------\n")
            
            # Archivar el lote usado
            archive_path = file_path.replace("data.csv", f"data_archived_{run.info.run_id}.csv")
            try:
